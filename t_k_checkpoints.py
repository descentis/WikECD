...
Updated t.py for the case of k_checkpoints

We save every kth entry and every other instance m (not a multiple of k) is stored as 
diff(m , int(m / k)*k)

Retrieval happens in 2 steps :- 
1. Reach closest instance that is a multiple of k == i == int(m / k)*k
2. Apply patch diff(m , i)
"""
...
class wikiRetrieval(object):
    ...
    def wiki_knolml_converter(self, *args, **kwargs):
        ...
        if kwargs.get('output_dir')!=None:
            ...
            file_path = kwargs['output_dir']+'/'+file_name[:-7]+str(intervalLength)+'.knolml'
            ...
            for event, elem in context_wiki:
                ...
                if event == "end" and 'revision' in elem.tag:
                    ...
                    if m != intervalLength+1:
                        with open(file_path,"a",encoding='utf-8') as myFile:
                            _ = self.wiki_file_writer(elem=elem,myFile=myFile,prefix=prefix,prev_str=prev_str,compression=compression)
    ...
    @staticmethod
    def wiki_decompress2(filename,k):
...
avg_len = int(sum(avg_list)/num_of_rev)
